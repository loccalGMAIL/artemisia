...
Auth: single `users` table/guard, no separate guard per panel. spatie/laravel-permission roles `staff`/`cliente` (via `HasRoles` on `User`) + nullable `cliente_id` decide access via `User::canAccessPanel()` — this is the real security boundary, not just the domain. Every business model has a policy (`app/Policies`) checked automatically by Filament; permissions follow the `{action}_{model}` convention (e.g. `view_any_cliente`), seeded idempotently by `database/seeders/RolesYPermisosSeeder.php`.
